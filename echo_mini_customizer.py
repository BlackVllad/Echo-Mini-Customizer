...
class FirmwareParser:
    ...
    def _update_loader_constants(self):
        ...
        layout constants.  Uses linear interpolation between two reference points:
          • HIFIEC320 (part5=22 968 838 B): CMP=67, A1=51,  A2=614, A4=921,  A3=1228
        ...
        # BUG-3 FIX: Guard the CMP write against reverting an already-patched theme dispatch.
        # For HIFIEC320-sized firmware delta≈0, lerp(67,0,0)=67=0x43, which would silently
        # un-apply the CMP R0,#0x00 patch. Only write if the byte hasn't been patched yet.
...
class EchoMiniCustomizer(QMainWindow):
    ...
    def _auto_load(self):
        ...
        # Check known filenames in app dir root first (both supported variants).
        for name in ("HIFIEC20.IMG", "HIFIEC320.IMG"):
            candidate = base / name
            if candidate.exists():
